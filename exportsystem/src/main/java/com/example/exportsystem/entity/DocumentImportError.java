...
// One row of the per-row error report for a bulk document import job. jobExecutionId
// points at Spring Batch's own BATCH_JOB_EXECUTION table (id), not a local FK - Batch
// owns that schema, we just reference it by id. See batch.DocumentImportSkipListener.
@Entity
@Table(name = "document_import_errors")
