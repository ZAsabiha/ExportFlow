    ...
    return (
        <DashboardLayout>
            ...
            <div className="dashboard-grid" style={{ marginTop: "25px" }}>
                <div className="panel" style={{ border: "2px solid var(--blue-500)", background: "var(--slate-50)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "15px" }}>
                        <KeyRound size={24} color="var(--blue-600)" />
                    ...
                    <form onSubmit={handleVerifyToken} style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
                        <input 
                            ...
                            style={{ flex: 1, padding: "12px", borderRadius: "8px", border: "1px solid var(--slate-300)", outline: "none", fontSize: "14px", fontFamily: "var(--mono)", fontWeight: 700 }}
                        />
                    ...
                    {verificationMessage && (
                        <div style={{
                            ...
                            fontSize: "13px",
                            fontWeight: 600,
                            background: verificationMessage.startsWith("Token validated") ? "var(--green-100)" : "var(--red-100)",
                            color: verificationMessage.startsWith("Token validated") ? "var(--green-700)" : "var(--red-800)"
                        }}>
                            {verificationMessage}
                        </div>
                    )}
                ...
                <div className="panel">
                    ...
                    <div style={{ fontSize: "13px", color: "var(--slate-600)", lineHeight: 1.6, marginTop: "10px" }}>
                        <p style={{ margin: "0 0 8px 0" }}>
                            <ShieldCheck size={16} color="var(--emerald-600)" style={{ verticalAlign: "middle", marginRight: "6px" }} />
            ...
            <div className="panel table-panel" style={{ marginTop: "30px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    {activeToken && <span style={{ fontSize: "12px", background: "var(--green-100)", color: "var(--green-700)", padding: "4px 10px", borderRadius: "12px", fontWeight: 700 }}>
                        Active Token Verified
                    </span>}
                </div>

                {downloadError && <p role="alert" style={{ color: "var(--red-700)" }}>{downloadError}</p>}
                ...
                <table style={{ marginTop: "15px" }}>
                    ...
                    <tbody>
                        ...
                        {unlockedDocuments.map((doc) => (
                            <tr key={doc.id}>
                                <td style={{ fontWeight: 700, color: "var(--slate-800)" }}>{doc.fileName}</td>
                                <td>
                                    <span style={{ padding: "3px 8px", borderRadius: "6px", fontSize: "12px", fontWeight: 600, background: "var(--indigo-100)", color: "var(--indigo-800)" }}>
                                        {doc.documentType?.replaceAll("_", " ")}
                                    </span>
                                </td>
                                <td><span style={{ fontWeight: 600, color: "var(--blue-600)" }}>{doc.orderCode}</span></td>
                                <td style={{ color: "var(--slate-500)" }}>{doc.fileSizeBytes ? `${Math.ceil(doc.fileSizeBytes / 1024)} KB` : "-"}</td>
                                <td style={{ color: "var(--slate-500)" }}>{doc.uploadedAt?.slice(0, 10)}</td>
                                <td>
